...
module.exports = Geneva = function(api, store) {
  ...
  if (_.isEmpty(api)) {
    ...
    api.addFunc("mod", function(args) {
      return api.run(args[0]) % api.run(args[1]);
    });
    ...
    api.addFunc("cond", function(args) {
      var conds = args,
          keys = _.range(0, conds.length, 2);
      return _.reduce(keys, function(result, key) {
        if (conds[key] === ":else") return api.run(conds[key+1]);
        if (!result) {
          if (!!api.run(conds[key])) {
            return api.run(conds[key+1]);
          }
        }
        return result;
      }, null);
    ...
    api.addFunc("concat", function(args) {
      return _.reduce(args, function(result, arg) {
        return result.concat(api.run(arg));
      });
    });

    api.addFunc("range", function(args) {
