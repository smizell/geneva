...
class Scope {
  ...
  getRef(refName) {
    ...
    const ref = mori.find(this.refs, refName);
    if (ref) {
      return mori.get(ref, 1);
    }
  ...
  setRef(name, value) {
    this.refs = mori.assoc(this.refs, name, value);
  }
};
