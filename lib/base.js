  ...
  if (_.isEmpty(api)) {
    ...
    api.addFunc("fn", function(args) {
      ...
      return function(fnArgs) {
        var zippedArgs = _.zipObject(argNames, fnArgs),
            newGeneva = Geneva(api, store);

        // Get values and put them into local storage
        _.each(Object.keys(zippedArgs), function(key) {
          var val = zippedArgs[key];
          ...
          if (newGeneva.isVar(val)) {
            ...
            newGeneva.setLocal(key, varObj.type, varObj.value);
          } else {
            newGeneva.setLocal(key, "var", val);
          };
        });
