...
const defaultForms = {
  ...
  templateFile: formWrapper((runtime, args) => {
    const arg = args[0];
  ...
  include: formWrapper((runtime, args) => {
    const arg = args[0];
    const filePath = runtime.run(arg);
  ...
  readFile: formWrapper((runtime, args) => {
    const arg = args[0];
    const filePath = runtime.run(arg);
...
class ObjectParser {
  ...
  parseCode(code) {
    ...
    const argsValue = code[fnWithChar];
    // Allow a single value or an array of values
    // This means an array can't be considered a single value, though
    const args = RA.isArray(argsValue) ? argsValue : [argsValue];
...
class ConfigBuilder {
  ...
  static fromYAML(yamlConfig, options) {
    return new ConfigBuilder({ ...options, config: YAML.parse(yamlConfig) });
  }
