...
const defaultForms = {
  ...
  lambda: new Form((parentRuntime, lambdaArgs) => {
    ...
    const storedRefs = R.clone(parentRuntime.scope.refs);
    const storedForms = R.clone(parentRuntime.scope.forms);

    // Each function call gets its own run time with its own scope
    return (...args) => {
      const refs = R.clone(storedRefs);
      const forms = R.clone(storedForms);
      const scope = new Scope(refs, forms, parentRuntime.scope.standardLib);
      ...
      // forEachIndex(([value, index]) => {
      // }, args.slice(0, argNames.length));
