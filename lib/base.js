...
module.exports = Geneva = function(scope) {
  ...
  api.setCoreFn("eval", function(args, global, local) {
    var code = global.run(args[0], local),
        funcName = code[0],
        func = global.get(funcName),
        newArgs = code.slice(1);
    return func.call(newArgs, global, local);
  });

  api.setCoreFn("->>", function(args, global, local) {
    return _.reduce(args, function(result, arg) {
      return global.run(arg.concat([result]), local);
    });
  });
  ...
  api.setCoreFn("and", function(args, global, local) {
    // Return false as soon as a false item is found
    for (i=0; i < args.length; i++) {
      if (!global.run(args[i], local)) return false;
    }
    return true
  });

  api.setCoreFn("or", function(args, global, local) {
    // Return true as soon as a true item is found
    for (i=0; i < args.length; i++) {
      if (global.run(args[i], local)) return true;
    }
    return false
  });
  ...
  api.setCoreFn("first", function(args, global, local) {
    var list = global.run(args[0], local);
    return _.first(list);
  });

  api.setCoreFn("nth", function(args, global, local) {
    var list = global.run(args[0], local),
        index = global.run(args[1], local);
    return list[index];
  });

  api.setCoreFn("conj", function(args, global, local) {
    var list = global.run(args[0], local),
        items = global.run(args.slice(1), local);
    if (_.isNull(list)) return items;
    return list.concat(items);
  });

  api.setCoreFn("drop", function(args, global, local) {
    var num = global.run(args[0], local),
        list = global.run(args[1], local);
  ...
  api.setCoreFn("true?", function(args, global, local) {
    var value = global.run(args[0], local);
    return !!value;
  });

  api.setCoreFn("false?", function(args, global, local) {
    var value = global.run(args[0], local);
    return !value;
  });

  api.setCoreFn("empty?", function(args, global, local) {
    var value = global.run(args[0], local);
    return value.length === 0;
  });

  api.setCoreFn("every?", function(args, global, local) {
    var func = global.run(args[0], local),
        list = global.run(args[1], local);
    return _.reduce(list, function(result, item) {
      if (func.call([item], global, local)) return result;
      return false;
    }, true);
  });

  api.setCoreFn("nil?", function(args, global, local) {
    var value = global.run(args[0], local);
  ...
  api.setCoreFn("array?", function(args, global, local) {
    var value = global.run(args[0], local);
    return _.isArray(value);
  });

  api.setCoreFn("string?", function(args, global, local) {
    var value = global.run(args[0], local);
    return _.isString(value);
  });

  api.setCoreFn("object?", function(args, global, local) {
    var value = global.run(args[0], local);
    return _.isObject(value);
  });

  api.setCoreFn("number?", function(args, global, local) {
    var value = global.run(args[0], local);
    return _.isNumber(value);
  });

  api.setCoreFn("bool?", function(args, global, local) {
    var value = global.run(args[0], local);
