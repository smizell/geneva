...
class Geneva {
  constructor(options = {}) {
    ...
    this.standardLib = options.standardLib || lodash;
  }

  run(code) {
    const scope = new Scope(this.initial, this.forms, this.standardLib);
  ...
  buildRuntime(options = {}) {
    ...
    const standardLib = options.standardLib || lodash;
    const scope = new Scope(initial, forms, standardLib);
