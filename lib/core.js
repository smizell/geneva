...
module.exports.Geneva = Geneva = function() {
  ...
  // Shorthand for getting the function
  funcs.gf = function(funcName) {
  ...
  funcs.inc = function(args) {
    var value = _.parseInt(funcs.valueOrCall(args[0]));
    return value + 1;
  }
  ...
  funcs["map"] = function(args) {
    var func = this.valueOrCall(args[0]),
        values = this.valueOrCall(args[1]);
